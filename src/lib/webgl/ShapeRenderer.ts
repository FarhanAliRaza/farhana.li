import { vertexShader as vertexShaderSource, fragmentShader as fragmentShaderSource } from './shaders';
...
export class ShapeRenderer {
    ...
    private readonly touchInfluenceRadius = 0.75; // Larger influence radius for touch
    private readonly touchInfluenceStrength = 0.7; // Stronger influence for touch
    
    private readonly predefinedShapes = new Set([
        'imac', 'heart', 'bulb', 'semicolon', 'x',
        'greater', 'curly-brackets', 'hash', 'camera', 'gear'
    ]);
    ...
    private updatePoints() {
        ...
        for (let i = 0; i < this.currentData.length / 3; i++) {
            ...
            this.temporaryArray[idx] += (this.jitterArray[idx] - this.temporaryArray[idx]) * this.dampingFactor;
            this.temporaryArray[idx + 1] += (this.jitterArray[idx + 1] - this.temporaryArray[idx + 1]) * this.dampingFactor;
            this.temporaryArray[idx + 2] += (this.jitterArray[idx + 2] - this.temporaryArray[idx + 2]) * this.dampingFactor;

            // Calculate transformed point with jitter
            const transformedPoint = [
                this.currentData[idx] * cosineValue - this.currentData[idx + 2] * sineValue + this.temporaryArray[idx],
                this.currentData[idx + 1] + this.temporaryArray[idx + 1],
                this.currentData[idx] * sineValue + this.currentData[idx + 2] * cosineValue + this.temporaryArray[idx + 2]
            ];
            ...
            if (distanceSquared < 1) {
                ...
                this.pointData[idx + 2] = currentPoint[2] + (transformedPoint[2] - currentPoint[2]) * easingFactor - 7;
            }
        }
    }
    ...
    private async loadNewShape() {
        ...
        const shapeName = [...this.predefinedShapes][Math.floor(Math.random() * this.predefinedShapes.size)];
    ...
    private createPerspectiveMatrix(fieldOfView: number, aspect: number, near: number, far: number): number[] {
