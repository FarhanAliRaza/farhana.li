...
const paths = import.meta.glob<{ default: Component; metadata: Omit<Log, 'slug'> }>(
	'/src/content/logs/*.md',
	{ eager: true }
);
...
export const load: PageLoad = async ({ params }) => {
	try {
		...
		const log = paths[`/src/content/logs/${params.slug}.md`];
		if (!log) throw error(404, `Could not find ${params.slug}`);
		...
		const relatedLogs = findRelatedLogs(params.slug, log.metadata.tags, allLogs);

		return {
			content: log.default,
			meta: { ...log.metadata, slug: params.slug },
			nextLogs,
			relatedLogs
		};
	} catch {
